...
#define HIST_LEN 15     // original: 16
#define TOUR_LEN 15     // original: 16
#define BHT_BIT_SIZE 12 // original: 11
...
void PREDICTOR::UpdatePredictor(UINT32 PC, bool resolveDir, bool predDir, UINT32 branchTarget)
{
  ...
  if (predictorChooseCounter[pCC] < (TOURNAMENT_CTR_MAX / 2 + 1))
  {
    ...
    if (global_pred_result == predDir && local_pred_result != predDir)
    {
      if (predictorChooseCounter[pCC] > 0)
        predictorChooseCounter[pCC]--;
    }
  }
  else
  {
    ...
    if (global_pred_result != predDir && local_pred_result == predDir)
    {
      if (predictorChooseCounter[pCC] < TOURNAMENT_CTR_MAX)
        predictorChooseCounter[pCC]++;
    }
  }
